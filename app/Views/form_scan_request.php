...
<body class="hold-transition layout-top-nav">
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0 text-secondary"> Order Picking Up List </SMALL></h1>
                        </div><!-- /.col -->
                        <div class="col-sm-6">
                                ...
                                <form class="form-horizontal">
                                    <div class="card-header">
                                        <div class="form-group row">
                                            <label for="rqOrder" class="col-md-1 col-form-label text-right">Scan Order</label>
                                            <div class="col-md-3">
                                                <input type="text" class="form-control form-control" id="rqOrder" name="rqOrder" placeholder="Please Scan Your Order Number" autofocus>
                                            </div>
                                        </div>
                                        <div class="form-group row">
                                            <label for="rqItem" class="col-md-1 col-form-label text-right">Scan Item</label>
                                            <div class="col-md-3">
                                                <input type="text" class="form-control form-control" id="rqItem" name="rqItem" placeholder="Please Scan Item" autofocus>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="card-body">
                                        ...
                                        <div class="table responsive">
                                            <table class="table table-striped table-hover" width="100%" id="tbScan">
                                                <thead>
                                                    <tr>
                                                        <th>Item Code</th>
                                                        <th>Description</th>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#rqOrder').blur(function() {
                var rqOrder = $('#rqOrder').val();

                if (rqOrder == "") {
                    Swal.fire({
                        icon: 'error',
                        title: 'Oops...',
                        text: 'Please Scan Order!'
                    });
                }

                $.ajax({
                    url: "<?= base_url("/getrequest") ?>/" + rqOrder,
                    type: 'GET',
                    dataType: 'JSON',
                    success: function(response) {
                        document.getElementById("rqOrder").setAttribute("readonly", "true");
                        console.log(response);
                        var html = '';
                        var i;
                        for (i = 0; i < response.length; i++) {
                            html += '<tr>' +
                                '<td><input type="text" class="form-control" name="rqdItem[]" id="prt' + response[i].rqd_itmid + '" readonly="true" value="' + response[i].rqd_itmid + '"></td>' +
                                '<td><input type="text" class="form-control" name="rqdDesc[]" readonly="true" value="' + response[i].rqd_desc + '"></td>' +
                                '<td><input type="text" class="form-control text-right" name="rqdOrQty[]" id="ord' + response[i].rqd_itmid + '" readonly="true" value="' + response[i].rqd_orderqty + '"></td>' +
                                '<td><input type="text" class="form-control text-right" name="rqdRcQty[]" id="rcv' + response[i].rqd_itmid + '" readonly="true" value="' + response[i].rqd_receiptqty + '"></td>' +
                                '<td><input type="text" class="form-control" name="rqdUnit[]" readonly="true" value="' + response[i].rqd_unit + '"></td>' +
                                '<td><input type="text" class="form-control" name="rqdStatus[]" readonly="true" value="' + response[i].rqd_status + '"></td>' +
                                '</tr>';
                        }
                        $('#list-data').html(html);
                    }
            ...
            $('#rqItem').blur(function() {
                var itemScan = $('#rqItem').val();

                if (itemScan == "") {
                    Swal.fire({
                        icon: 'error',
                        title: 'Oops...',
                        text: 'Please Scan Item!'
                    });
                }

                $.ajax({
                    url: "<?= base_url("/getrequest") ?>/" + $('#rqOrder').val(),
                    type: 'GET',
                    dataType: 'JSON',
                    success: function(response) {
                        var found = 0;
                        var line;
                        for (i = 0; i < response.length; i++) {
                            if (response[i].rqd_itmid == itemScan) {
                                found++;
                            }
                        }

                        if (found > 0) {
                            console.log('item detect');
                            var qb = parseInt($('#rcv' + itemScan).val());
                            var qo = parseInt($('#ord' + itemScan).val());
                            if (qb < qo) {
                                $('#rcv' + itemScan).val(qb + 1);
                                $('#rqItem').val('');
                                $('#rqItem').focus();
                            } else {
                                Swal.fire({
